...
//https://open.weixin.qq.com/connect/oauth2/authorize?appid=企业号ID&redirect_uri=http://服务器域名/getuserid.php&response_type=code&scope=snsapi_privateinfo&agentid=1000002&state=1#wechat_redirect
	$code = $_GET["code"];  
	...
	if(!empty($userid))//如果有userid，表示是企业用户
	{
		...
		$_SESSION['alarm_user_name'] = $user_obj['name'];
		$_SESSION['alarm_user_email'] = $user_obj['email'];
		$_SESSION['alarm_user_mobile'] = $user_obj['mobile'];
		$url="./alminfosubs/SubscribeUI.php";//跳转到报警订阅设置页面
		header("Location: {$url}"); 
